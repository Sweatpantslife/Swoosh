...
Original music + SFX for **Swoosh**, a mobile-first match-3 puzzle game with a
premium casino aesthetic (jewel tones, neon, "slot-machine excitement meets
puzzle elegance"). All files generated with Higgsfield (`sonilo_music` for the
BGM, `mirelo_text_to_audio` for SFX), then trimmed / normalized / encoded with
ffmpeg.
...
|------|---------|---------:|-----:|-----------|--------------------:|
| `bgm-main-loop.mp3` | Background music — elegant casino-lounge with modern electronic sheen; upbeat but non-fatiguing over long sessions | 76.07 s | 1.49 MB | **loop** | 0.55 |
...
| `sfx-color-bomb.mp3` | Color bomb — dramatic charged blast with jackpot sparkle | 2.53 s | 40 KB | one-shot | 0.95 |
...
| `sfx-level-win.mp3` | Level win — celebratory jackpot fanfare | 3.40 s | 54 KB | one-shot | 0.90 |
...
- HTML5 `<audio loop>` also works but the element may insert a tiny gap at the
  wrap; prefer Web Audio API for a truly seamless lounge bed.
